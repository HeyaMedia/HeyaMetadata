...
func (s *Service) merge(ctx context.Context, normalizedID string, additionalNormalizedIDs []string, normalized moviedomain.NormalizedRecordV1, successfulRecords []moviedomain.NormalizedRecordV1, riverJobID int64) (Result, error) {
	...
	sort.SliceStable(projection.Detail.Data.Credits, func(i, j int) bool {
		left, right := projection.Detail.Data.Credits[i], projection.Detail.Data.Credits[j]
		return people.CanonicalizationOrder(left.Provider, left.ProviderPersonID) < people.CanonicalizationOrder(right.Provider, right.ProviderPersonID)
	})
