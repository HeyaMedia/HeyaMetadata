...
func (s *Service) merge(ctx context.Context, normalizedIDs []string, successful []rgdomain.NormalizedRecordV1, jobID int64) (Result, error) {
	...
	for _, input := range records {
		for _, edition := range input.Record.Editions {
			...
			if _, err := tx.Exec(ctx, `INSERT INTO entity_relations(source_entity_id,target_entity_id,source_kind,target_kind,relation_type,provider,namespace,provider_value,metadata,state,source_observation_id,first_observed_at,last_observed_at) VALUES($1,(SELECT claim.entity_id FROM external_id_claims claim JOIN entities entity ON entity.id=claim.entity_id AND entity.kind='release' AND entity.deleted_at IS NULL WHERE claim.entity_kind='release' AND claim.provider=$2 AND claim.namespace=$3 AND claim.normalized_value=$4 AND claim.state='accepted' LIMIT 1),'release_group','release','editions',$2,$3,$4,$5,'accepted',$6,$7,$7) ON CONFLICT(source_entity_id,relation_type,provider,namespace,provider_value) DO UPDATE SET target_entity_id=EXCLUDED.target_entity_id,metadata=EXCLUDED.metadata,state='accepted',source_observation_id=EXCLUDED.source_observation_id,last_observed_at=EXCLUDED.last_observed_at`, entityID, edition.Provider, edition.Namespace, edition.ProviderID, metadata, input.Record.ProviderRecord.PrimaryObservationID, input.Record.ProviderRecord.ObservedAt); err != nil {
