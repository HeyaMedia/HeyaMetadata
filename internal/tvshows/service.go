...
func normalize(payload providers.Payload) (episodic.NormalizedRecord, error) {
	...
	if value.Image.Original != "" {
		record.Images = append(record.Images, episodic.Image{ProviderID: "show-original", URL: value.Image.Original, Class: "poster"})
	}
	for _, image := range value.Embedded.Images {
		for resolution, item := range image.Resolutions {
			if item.URL != "" {
				record.Images = append(record.Images, episodic.Image{ProviderID: fmt.Sprintf("%d-%s", image.ID, resolution), URL: item.URL, Class: normalizeType(image.Type), Width: item.Width, Height: item.Height})
			}
		}
	}
	return record, nil
