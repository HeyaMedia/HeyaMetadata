...
const MergeVersion = "book-combiner/v1"
...
func (s *Service) IngestWork(ctx context.Context, key string, jobID int64, credentials providercredentials.Credentials) (result Document, returnErr error) {
	...
	if jobID > 0 {
		_, returnErr = s.runtime.DB.Exec(ctx, `INSERT INTO book_ingestion_runs(river_job_id,openlibrary_work_id,state)VALUES($1,$2,'working')ON CONFLICT(river_job_id)DO UPDATE SET state='working',error=NULL,completed_at=NULL`, jobID, key)
		if returnErr != nil {
	...
	if e == nil {
		...
		for _, ed := range editions.Entries[:min(len(editions.Entries), 15)] {
	...
	return s.persist(ctx, key, work, workPayloads[0], editions.Entries, edPayload, authors, authorPayloads, google, googleObs, jobID)
}

func (s *Service) persist(ctx context.Context, key string, work olWork, wp providers.Payload, editions []olEdition, ep providers.Payload, authors []Author, authorPayloads map[string]providers.Payload, google map[string]googleVolumes, googleObs map[string]string, jobID int64) (Document, error) {
	...
	workID, slug, created, err := ensureEntity(ctx, tx, "book_work", "openlibrary", "work", key, slugify(work.Title, year(work.FirstPublishDate), "book"), wp.ObservationID, wp.ObservedAt)
	if err != nil {
		return Document{}, err
	}
	if err = persistNormalized(ctx, tx, workID, "book_work", "openlibrary", "work", key, wp.ObservationID, "openlibrary-work/v1", work, wp.ObservedAt); err != nil {
		return Document{}, err
	}
	if ep.ObservationID != "" {
		if err = persistNormalized(ctx, tx, workID, "book_work", "openlibrary", "work_editions", key, ep.ObservationID, "openlibrary-editions/v1", editions, ep.ObservedAt); err != nil {
			return Document{}, err
		}
	}
	var version int64
	...
	doc := Document{SchemaVersion: 1, ProjectionVersion: version, ID: workID, Kind: "book_work", Slug: slug, ExternalIDs: []ExternalID{{Provider: "openlibrary", Namespace: "work", Value: key}}, Freshness: fresh, Provenance: map[string][]SourceRef{"work": {{Provider: "openlibrary", ObservationID: wp.ObservationID}}}}
	...
	for _, ed := range editions {
		...
		eid, eslug, _, e := ensureEntity(ctx, tx, "book_edition", "openlibrary", "edition", ek, slugify(first(ed.Title, work.Title), year(ed.PublishDate), "edition"), ep.ObservationID, ep.ObservedAt)
		if e != nil {
			return Document{}, e
		}
		...
		edoc := Document{SchemaVersion: 1, ProjectionVersion: 1, ID: eid, Kind: "book_edition", Slug: eslug, ExternalIDs: []ExternalID{{Provider: "openlibrary", Namespace: "edition", Value: ek}}, Freshness: fresh, Provenance: map[string][]SourceRef{"edition": {{Provider: "openlibrary", ObservationID: ep.ObservationID}}}}
		...
		editionImages, imageErr := s.persistCovers(ctx, tx, eid, ed.Covers, ep.ObservationID)
		if imageErr != nil {
		...
		for _, isbn := range append(append([]string{}, ed.ISBN10...), ed.ISBN13...) {
			...
			if _, e = tx.Exec(ctx, `INSERT INTO external_id_claims(entity_id,entity_kind,provider,namespace,normalized_value,state,source_observation_id,first_observed_at,last_observed_at)VALUES($1,'book_edition','isbn',$2,$3,'accepted',$4,$5,$5)ON CONFLICT(entity_kind,provider,namespace,normalized_value)DO UPDATE SET last_observed_at=EXCLUDED.last_observed_at WHERE external_id_claims.entity_id=EXCLUDED.entity_id`, eid, ns, normalizeISBN(isbn), ep.ObservationID, ep.ObservedAt); e != nil {
				return Document{}, fmt.Errorf("persist edition ISBN: %w", e)
			}
		}
		if values := google[ek]; len(values.Items) > 0 {
			g := values.Items[0]
			if e = persistNormalized(ctx, tx, eid, "book_edition", "googlebooks", "volume", g.ID, googleObs[ek], "googlebooks-volume/v1", g, now); e != nil {
				return Document{}, e
			}
			...
			if _, e = tx.Exec(ctx, `INSERT INTO external_id_claims(entity_id,entity_kind,provider,namespace,normalized_value,state,source_observation_id,first_observed_at,last_observed_at)VALUES($1,'book_edition','googlebooks','volume',$2,'accepted',$3,$4,$4)ON CONFLICT(entity_kind,provider,namespace,normalized_value)DO UPDATE SET last_observed_at=EXCLUDED.last_observed_at WHERE external_id_claims.entity_id=EXCLUDED.entity_id`, eid, g.ID, googleObs[ek], now); e != nil {
	...
	_, _ = tx.Exec(ctx, `INSERT INTO change_outbox(entity_id,entity_kind,slug,change_type,changed_scopes,projection_version,provider_observation_id,river_job_id)VALUES($1,'book_work',$2,$3,$4,$5,$6,$7)`, workID, slug, change, []string{"identity", "detail", "search", "provenance"}, version, wp.ObservationID, nullable(jobID))
	if jobID > 0 {
...
func (s *Service) OpenLibraryWorkID(ctx context.Context, id string) (string, error) {
	var v string
	err := s.runtime.DB.QueryRow(ctx, `SELECT normalized_value FROM external_id_claims WHERE entity_id=$1 AND entity_kind='book_work' AND provider='openlibrary' AND namespace='work' AND state='accepted'`, id).Scan(&v)
	return v, err
}

func (s *Service) persistCovers(ctx context.Context, tx pgx.Tx, entityID string, covers []int64, observationID string) ([]Image, error) {
	images := []Image{}
	for _, cover := range covers[:min(len(covers), 20)] {
		...
		var imageID string
		if err := tx.QueryRow(ctx, `INSERT INTO image_candidates(entity_id,provider,provider_image_id,class,source_url,source_observation_id)VALUES($1,'openlibrary',$2,'cover',$3,$4)ON CONFLICT(entity_id,provider,provider_image_id,class)DO UPDATE SET source_url=EXCLUDED.source_url,source_observation_id=EXCLUDED.source_observation_id RETURNING id`, entityID, providerID, sourceURL, observationID).Scan(&imageID); err != nil {
...
func persistNormalized(ctx context.Context, tx pgx.Tx, entityID, kind, provider, namespace, providerID, observationID, version string, document any, observed time.Time) error {
	...
	_, err = tx.Exec(ctx, `INSERT INTO normalized_records(entity_id,entity_kind,provider,provider_namespace,provider_record_id,primary_observation_id,normalizer_version,schema_version,document,observed_at)VALUES($1,$2,$3,$4,$5,$6,$7,1,$8,$9)ON CONFLICT(primary_observation_id,normalizer_version,schema_version)DO UPDATE SET entity_id=EXCLUDED.entity_id,document=EXCLUDED.document`, entityID, kind, provider, namespace, providerID, observationID, version, body, observed)
	return err
}
