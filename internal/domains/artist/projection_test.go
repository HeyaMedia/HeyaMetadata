...
func TestCombineUsesMusicBrainzSpineAndRetainsSupplementalEvidence(t *testing.T) {
	...
	lastfm := NormalizedRecordV1{ProviderRecord: ProviderRecord{Provider: "lastfm", PrimaryObservationID: "lastfm", ObservedAt: now}, Metrics: []Metric{{Name: "listeners", Value: 6000000}}, SimilarArtists: []SimilarArtist{{Name: "John Lennon"}}}
	...
	if projection.Detail.Display.Name != "The Beatles" || projection.Detail.Display.ImageID != "opaque-image" || len(projection.Detail.ExternalIDs) != 2 || len(projection.Detail.Data.Metrics) != 1 || len(projection.Detail.Data.Relationships) != 1 {
