...
func TestNormalizeRecordingPreservesRecordingEvidence(t *testing.T) {
	...
	if value.ProviderID == "" || len(value.ISRCs) != 1 || len(value.ArtistCredits) != 1 || len(value.Releases) != 1 || len(value.Genres) != 1 || len(value.Tags) != 1 || value.Rating == nil || len(value.Links) != 1 {
